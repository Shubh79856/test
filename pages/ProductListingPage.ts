...
export class ProductListingPage {
  ...
  async getSelectedSubCategoryTexts_FilterByCategory(): Promise<string[]> {
    await this.selectedSubCategoryName_FilterByCategory
      .last()
      .waitFor({ state: "attached", timeout: 7 * 1000 });
    ...
    console.log("Breadcrumb texts:", allValues);
  ...
  async selectSubcategoryName_FilterByCategory(
    ...
    return await test.step(`Select the '${subcategoryName}' filter under the Filter By Category section`, async () => {
      ...
      throw new Error(
        `Subcategory/Filter '${subcategoryName}' not found under Filter By Category section`
      );
    });
  }
  ...
  async clickListViewMode(): Promise<void> {
    await test.step("Click on the 'List View' icon at the very top of the search results page", async () => {
      await this.listViewMode.waitFor({ state: "visible", timeout: 7 * 1000 });
  ...
  async isProductAddedSuccessMessageDisplayed(
    ...
    try {
      const notificationLocator = await this.page.locator(".message-success");
  ...
  async adjustPriceSlider_FilterByPrice(
    ...
    await slider.waitFor({ state: "visible", timeout: 10 * 1000 });
  ...
  async isURLendWith(queryParam): Promise<boolean> {
    await this.page.waitForLoadState("domcontentloaded");
